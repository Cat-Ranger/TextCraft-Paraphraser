...
// Main execution handler with automatic retry for high demand spikes
async function processText() {
  const text = sourceText.value.trim();
  if (!text) return;

  // 1. Show loading state based on active mode
  ...
  // 2. Build prompt dynamically based on mode
  let prompt = "";
  ...
  while (retryCount <= maxRetries && !success) {
    try {
      ...
      // Handle Server Overload (500/503) or Rate Limit (429) BEFORE parsing JSON
      if (
        response.status === 429 ||
        response.status === 503 ||
        response.status === 500
        ...
        if (retryCount <= maxRetries) {
          resultContent.textContent = `High demand on server. Retrying (${retryCount}/${maxRetries})...`;
          await wait(1500 * retryCount); // Wait 1.5s, 3s, 4.5s
          continue;
        }
      }
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        throw new Error(
          `Server returned non-JSON response (${response.status}): ${textError.substring(0, 100)}`,
        );
      }
      ...
      if (!response.ok) {
        ...
        resultContent.textContent = `API Error: ${apiErrorMessage}`;
        return;
      }
      ...
      if (retryCount >= maxRetries) {
        resultContent.textContent = `Error: ${error.message}. Make sure node server.js is running!`;
      }
      retryCount++;
      await wait(1500);
    }
